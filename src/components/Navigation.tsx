    ...
    <>
      ...
      >
        <div className="p-4 sm:p-6 space-y-2">
          {navItems.map((item, idx) =>
            item.submenu ? (
              <div key={idx} className="rounded-xl overflow-hidden bg-white/5">
                <button
              ...
              <Link
                key={item.name}
                to={item.path}
                className="flex items-center gap-3 px-4 py-3.5 text-white hover:bg-white/10 rounded-xl transition-all duration-200 active:scale-98"
                onClick={handleNavClick}
              >
                <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-white/10">
                  <item.icon size={20} className="text-blue-200" />
